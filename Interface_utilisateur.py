...
# Permet de gérer le blocage et déblocage des boutons si le fichier est
# chargé ou non
...
# Récupération du nombre d'individus et de générations souhaités
...
Button_open = Button(
    main, text='Ouvrir', command=lambda: [
        filename.set(
            askopenfilename(
                filetypes=[
                    ("Fasta file", "*.fasta*")])), switchButtonState()])
...
Label(
    main,
    text="Entrer une valeur de probabilité (float)").grid(
        row=7,
    column=0)
...
# Permet de lancer les calculs d'optimisation d'angles, tracer la
# convergence et afficher le brain d'ADN cyclique
Button_calculus = Button(
    main, text="Lancer les calculs", command=lambda: Princ.draw_traited(
        filename.get(), bool(
            Scalvar.get()), Methode.get(), int(
                nombre_generation.get()), int(
                    nombre_indiv.get()), float(
                        Luckvar.get()), int(
                            Puissvar.get())), state='disabled')
Button_calculus.grid(row=10, column=1)

# Permet de tracer le brain d'ADN à partir de la table initiale
Button_init = Button(
    main,
    text="Tracer la trajectoire 3D de la molécule initiale",
    command=lambda: Princ.draw_initial_seq(
        filename.get()),
    state='disabled')
Button_init.grid(row=10, column=0)

# création d'un boutton qui entraine la destruction (fermerture) de l'interface
Button(main, text="Quitter", command=main.destroy).grid(row=12, columnspan=3)
