...
filename = StringVar()
Label(main,text="Cliquez sur ouvrir").grid(row=2,column=0)
Entry(main, textvariable=filename,state='disabled').grid(row=2,column=1)
Button(main,text='Ouvrir',command=lambda:filename.set(askopenfilename())).grid(row=2,column=2)

Methode= StringVar(main)
Label(main, text= "Sélection de la méthode").grid(row = 4, column = 0)
ttk.Combobox(main,values=["Tournoi","Elitisme"],textvariable=Methode).grid(row=4,column=1)

######Attention ici changer Command None en lancement des claculs de main.py 
Button(main,text="Lancer les calculs",command=lambda : Princ.test_draw_traited(filename.get(),Methode.get(),int(nombre_generation.get())) ).grid(row=5, column= 1)

Button(main,text="Tracer la trajectoire 3D de la molécule initiale",command=lambda : Princ.test_draw_initial_seq(filename.get())).grid(row=6,column=0)
