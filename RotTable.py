...
class RotTable:
    ...
    def __init__(self, randomGen=False, rot_dict=None):
        if rot_dict is None:  # on peut créer un invididu de manière aléatoire en conservant le respect des bornes
            self.__Rot_Table = {}
            for dinucleotide in RotTable.__ORIGINAL_ROT_TABLE:
                self.__Rot_Table[dinucleotide] = []
                for i in range(3):
                    self.__Rot_Table[dinucleotide].append(
                        RotTable.__ORIGINAL_ROT_TABLE[dinucleotide][i])
                    # on initialise en ajoutant une valeur alétoire avec une
                    # loi qui respecte les contraintes d'intervalles
                    self.__Rot_Table[dinucleotide][i] += np.random.uniform(
                        -1, 1) * RotTable.__ORIGINAL_ROT_TABLE[dinucleotide][i + 3] if randomGen else 0
        else:
            if isinstance(
                    rot_dict,
                    dict):  # on peut créer un individu à partir d'un dictionnaire
                self.__Rot_Table = rot_dict
            else:
                raise Exception(
                    "Type of rot_dict is not correct, you should give a dictionnary")

    ###################
    # READING METHODS #
    ###################
    def __str__(self):  # on surcharge la méthode str pour afficher l'objet table comme on le désire
        ...
        for dinucle in self.__KEYS_UNIQUE:
            if self.Symetrique(dinucle) not in self.__Rot_Table.keys():
                ...
                for i in range(len(table[dinucle])):
                    if i != 2:
                        table[self.Symetrique(dinucle)].append(
                            table[dinucle][i])
                    else:
                        table[self.Symetrique(
    ...
    def Mutate(self, gen):
        ...
        if not (isinstance(gen, int)):
            raise Exception(
                "gen must be an integer which represents the current generation")
        dinucle = np.random.choice(list(self.__Rot_Table.keys()))
        # on modifie un angle au hasard parmi ceux qu'on peut modifier (que
        # deux choix)
        angle = np.random.randint(0, 2)
        # on récupère la variance de l'angle
        delta = self.__ORIGINAL_ROT_TABLE[dinucle][angle + 3]
    ...
    def Cross(self, rot_table_2, cut):
        ...
        if not (isinstance(cut, int) and cut in range(self.__NUM_KEYS)):
