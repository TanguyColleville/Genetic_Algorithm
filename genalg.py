...
parser.add_argument("-ng","--number_of_generation", type=int, default=5, help="Number of generations to go through")
...
parser.add_argument("-s", "--save", type=str, help="File location to save the best individual's RotTable dictionary")
parser.add_argument("--load", type=str, help="File location of a RotTable's dictionary to display a DNA sequence")
parser.parse_args()
args = parser.parse_args()

def make_evolution(seq, N_gen, N_ind, sel, scaling, alpha, luck, p):
    pop = Population(seq, N_ind)
    pop.evolve(N_gen, selection_method=sel, scaling=scaling, alpha=alpha, luck_prob=luck, puissance=p)
...
if args.mode == 'evolution':
    best = make_evolution(seq, args.number_of_individuals, args.number_of_generation, args.selection_method, args.scaling, args.alpha, args.luck, args.power)
